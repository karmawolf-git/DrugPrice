#!/usr/bin/env node
// manual-refresh: 2026-09-02 (9월 갱신 — 9/1 스케줄 무커밋 대응)
/**
...
const DRUG_CONFIGS = {
  ...
  'lipitor-plus': {
    ...
    //  - genericEdis: 알려진 상표명 제네릭 전체 목록 (mdsCd 직접 조회 → 완전 수집)
    // NOTE: autoDiscover(광범위 접두어 스캔)는 이 API에서 16분+ 소요돼 비실용적이라 미사용.
    //       genericEdis가 이미 전 상표명 제네릭을 커버하므로 불필요.
...
// 복합제 상표명 제네릭 접두어 (성분 무관 — gnlNmCd로 최종 필터링).
// itmNm 전방일치만 지원되므로 상표명 첫 음절을 커버.
// NOTE: 광범위 접두어는 페이지네이션 폭증으로 매우 느리므로 최소한으로 유지.
const DEFAULT_DISCOVER_PREFIXES = [
  '아모', '암로', '카두', '아암', '암아', '카암', '암카', '바스', '노바', '복합',
]
